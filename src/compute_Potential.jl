...
function Sph_with_Core_Phi_oe(Amplitude, r , radius_b, ξ, ε_r1, l_max)
    Phi_oe = zeros(Float64, l_max)
    E = calculate_ESph_with_Core(Amplitude, radius_b, ε_r1, l_max)
    for l in 0:l_max-1
        Phi_oe[l+1] =  ((E[l+1] * r^(-l-1))  * Pl(ξ,l) )  
    end
    return sum(Phi_oe) - (Amplitude* r * ξ)
...
function calculate_Phi_ox(Amplitude, r ,ra, rb, ξ, ε_r1, ε_r2, l_max)
    Phi_o = zeros(Float64, l_max)
    coefficients = calculate_LH_A2(Amplitude, ra, rb, ε_r1, ε_r2, l_max)
    E = coefficients[:,3]
    for l in 0:l_max-1
...
function calculate_Phi_oex(Amplitude, r ,ra, rb, ξ, ε_r1, ε_r2, l_max)
    Phi_oe = zeros(Float64, l_max)
    coefficients = calculate_LH_A2(Amplitude, ra, rb, ε_r1, ε_r2, l_max)
    E = coefficients[:,3]
    for l in 0:l_max-1
...
function calculate_Phi_PECx(Amplitude, ra ,  rb , ε_r1, ε_r2, l_max)
    coefficients = calculate_HH_A1(Amplitude, ra, rb, ε_r1, ε_r2, l_max)
    return coefficients[3,1]
end


function calculate_Phix(Amplitude, r, ra, rb, ξ, ε_r1, ε_r2, l_max)
    if r <= ra
