...
function calculate_LH_A2(Amplitude, radius_a, radius_b, ε_r1, ε_r2, l_max)  #lower Hiemspere
    ...
    for k in 0:l_max-1  
        for l in 1:l_max-1
            term1[k+1] = -Amplitude * radius_b * ( (1/(k+1)) + 1 + (((-1)^(1+k)) / (k+1)) + ((-1)^(1+k)) ) * U[2, k+1] 
            term2[1 , k+1 ] = (1+((-1)^k)) * U[1, k+1]
            ...
            #term2[l+1, k+1] = radius_b^(l) * (η[l+1] + ((η[l+1] * l * ε_r1 )/(k+1)) +  ((-1)^(k+l)) +  ( (((-1)^(k+l)) * l * ε_r2)  /  (k+1))  ) * U[l+1, k+1]          
        end
    end 
