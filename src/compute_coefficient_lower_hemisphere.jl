...
function Parts(radius_a, radius_b, ε_r1, ε_r2, l_max)
    ...
    for l in 0:l_max-1  
        for m in 0:l_max-1
            part1[l+1 , m+1 ] = U[l+1, m+1] * ((η[l+1] * (l+1) ) + ( ((-1)^(l+m)) * (l + 1) ) + ( η[l+1] * ε_r1 * l ) + ( ((-1)^(l+m)) * ε_r2 * l)) * (radius_b^l)
            part2[l+1 , m+1 ] = U[l+1, m+1] * ((radius_a^((2 * l) + 1)) * l * ( (1 - ε_r1) + ((1 - ε_r2) * (-1)^(l+m)) ) ) * (radius_b^(-l-1))
        end
    end 
    return part1 + part2
...
function calculate_LH_A2(Amplitude, radius_a, radius_b, ε_r1, ε_r2, l_max)  #lower Hiemspere
    ...
    for l in 0:l_max-1  
        for m in 0:l_max-1
            term1[l+1] = - Amplitude * U[l+1, 2] * ( radius_b * (1 + ( (-1)^(l+1) ) + (  ((-1)^(l+1)) * (l+1) )  + (l+1)     ))
